...
FILES = {
    ("1", "PT"): f"{B1}/Data/230807_PT.xls",
    ("1", "T1"): f"{B1}/Data/230808_T1.xls",
    ("1", "T2"): f"{B1}/Data/230809_T2.xls",
    ("1", "R1"): f"{B1}/Data/230810_R1.xls",
    ("1", "R2"): f"{B1}/Data/230811_R2.xls",
    ("2", "PT"): f"{B2}/Data/230904_PT.xls",
    ("2", "T1"): f"{B2}/Data/230905_T1.xls",
    ("2", "T2"): f"{B2}/Data/230906_T2.xls",
    ("2", "R1"): f"{B2}/Data/230907_R1.xls",
    ...
    cross.append(
        {
            ...
            "local_id_unique_globally": "no"
            if any(b["local_id"] == local and b["batch"] != batch for b in animals)
            else "yes",
            "collides_with": "; ".join(
                b["animal_key"] for b in animals if b["local_id"] == local and b["batch"] != batch
            ),
            "join_caveat": a["notes"],
            "evidence": a["evidence"],
        }
...
sessions_rows = []
for (batch, sess), rows in sorted(
    resume.items(), key=lambda kv: (kv[0][0], SESSIONS.index(kv[0][1]))
):
        ...
        sessions_rows.append(
            {
                ...
                "shock_amplitude_source": "parsed from the protocol filename string"
                if amp
                else "n/a (no shock)",
...
print(
    ...
    len(animals),
    "| batch1:",
    sum(1 for a in animals if a["batch"] == "1"),
    "batch2:",
    sum(1 for a in animals if a["batch"] == "2"),
)
import collections
...
print(
    "  colliding local ids:",
    sorted({c["local_id"] for c in cross if c["local_id_unique_globally"] == "no"}),
)
print("  join caveats:", [c["animal_key"] for c in cross if c["join_caveat"]])
print(
    "  animals with no biological id:", [a["animal_key"] for a in animals if not a["biological_id"]]
)
print(
    "  analysis inclusion unknown:",
    [
        a["animal_key"]
        for a in animals
